                ...
                builder.setPositiveButton("Borrar chat") { dialogo, _ ->
                    ...
                    collectionRef.document(user).collection("chats").document((Seleccion.get(info.position).id)).delete()
                        .addOnSuccessListener {
                            println((ListViews.adapter as ChatAdapter).getData()[info.position].id[1].toString())
                            println(collectionRef.document(user).collection("chats").document((Seleccion.get(info.position).id)).delete())
                        }
                        .addOnFailureListener { exception ->
                    ...
                    collectionRefe.document((Seleccion.get(info.position).id)).delete()
                        .addOnSuccessListener {
                            println((ListViews.adapter as ChatAdapter).getData()[info.position].id[1].toString())
                            println(collectionRefe.document((Seleccion.get(info.position).id)))
                        }
                        .addOnFailureListener { exception ->
                            println("Error al eliminar el documento: $exception")
                        }
    ...
    private fun newChat() {
        ...
        chatRef.get().addOnSuccessListener { messages ->
            ...
            if (listMessages.isEmpty()) {
                db.collection("Perfiles").document(otherUser).get().addOnSuccessListener { document ->
                    if (document.exists()) {
